...
async def analyze_audio(file_path: str = Form(...)):
    """
    Анализ аудиофайла по пути file_path
    """
    try:
...
def read_audio_from_path(file_path: str) -> Tuple[np.ndarray, int]:
    """
    Чтение существующего файла по file_path
    """
    ...
    if ext == ".flac":
        return read_flac_audio(raw)
    else:
        return convert_to_flac(raw)
...
def convert_to_flac(file_contents: bytes) -> Tuple[np.ndarray, int]:
    """
    Конвертация любого аудиоформата в FLAC
    """
...
def process_audio_data(audio_data: np.ndarray, sample_rate: int) -> Dict[str, str]:
    ...
    return format_prediction_result(score, features)
